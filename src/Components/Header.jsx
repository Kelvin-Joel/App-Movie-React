...
const Header = () => {
  ...
  return (
    <header className='header'>
      <img type="submit" id='lblinput' className='header-logo' src="https://image.tmdb.org/t/p/original/wwemzKWzjKYJFfCeiB57q3r4Bcm.svg" alt="" />
      ...
      <nav className='navbar'>
        <ul className='navbar-list'>
          <li style={{cursor:"pointer"}} className='navbar-item' onClick={()=>fetchMovies(1)}>Movies</li>
          <li className='navbar-item'>About</li>
          <li className='navbar-item btn-favorites' onClick={showListMoviesFavorite}><span className='total-movies'>{bd.length}</span> ❤</li>
        </ul>
      </nav>
    </header>
  )
